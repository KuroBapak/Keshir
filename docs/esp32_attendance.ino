...
String mqtt_topic_sub_response = "keshir/attendance/" + device_id + "/down/response";
...
void sendMqttConnect() {
  ...
  uint8_t varHeader[] = { 0x00, 0x04, 'M', 'Q', 'T', 'T', 0x04, 0xC2, 0x00, 0x3C };
  uint16_t clientLen = clientId.length();
  uint16_t userLen = strlen(mqtt_user);
  uint16_t passLen = strlen(mqtt_pass);
  size_t remainingLen = sizeof(varHeader) + (2+clientLen) + (2+userLen) + (2+passLen);
  uint8_t packet[256]; int idx = 0;
  packet[idx++] = 0x10; 
  size_t len = remainingLen; do { uint8_t d = len % 128; len /= 128; if (len > 0) d |= 0x80; packet[idx++] = d; } while (len > 0);
  for(int i=0; i<sizeof(varHeader); i++) packet[idx++] = varHeader[i];
  packet[idx++] = (clientLen >> 8); packet[idx++] = (clientLen & 0xFF); for(int i=0; i<clientLen; i++) packet[idx++] = clientId[i];
  packet[idx++] = (userLen >> 8); packet[idx++] = (userLen & 0xFF); for(int i=0; i<userLen; i++) packet[idx++] = mqtt_user[i];
  packet[idx++] = (passLen >> 8); packet[idx++] = (passLen & 0xFF); for(int i=0; i<passLen; i++) packet[idx++] = mqtt_pass[i];
  webSocket.sendBIN(packet, idx);
}

void sendMqttSubscribe(const char* topic) {
  String t = topic; size_t rLen = 5 + t.length();
  uint8_t packet[128]; int idx = 0;
  packet[idx++] = 0x82; packet[idx++] = rLen;
  packet[idx++] = 0x00; packet[idx++] = 0x01; 
  packet[idx++] = (t.length() >> 8); packet[idx++] = (t.length() & 0xFF);
  for(int i=0; i<t.length(); i++) packet[idx++] = t[i];
  packet[idx++] = 0x00;
  webSocket.sendBIN(packet, idx);
}

void sendMqttPublish(const char* topic, const char* payload) {
  String t = topic; String p = payload;
  size_t rLen = 2 + t.length() + p.length();
  uint8_t packet[512]; int idx = 0;
  packet[idx++] = 0x30; 
  size_t len = rLen; do { uint8_t d = len % 128; len /= 128; if (len > 0) d |= 0x80; packet[idx++] = d; } while (len > 0);
  packet[idx++] = (t.length() >> 8); packet[idx++] = (t.length() & 0xFF); for(int i=0; i<t.length(); i++) packet[idx++] = t[i];
  for(int i=0; i<p.length(); i++) packet[idx++] = p[i];
  webSocket.sendBIN(packet, idx);
}
    ...
    case WStype_BIN:
      if (length > 0) {
        if ((payload[0] & 0xF0) == 0x20) { 
           ...
           int h=2; if((payload[1]&0x80)!=0) h=3;
           int tLen = ((unsigned char)payload[h]<<8)|(unsigned char)payload[h+1];
           String topic=""; for(int i=h+2; i<h+2+tLen; i++) topic+=(char)payload[i];
           String msg=""; for(int i=h+2+tLen; i<length; i++) msg+=(char)payload[i];
           
           StaticJsonDocument<256> doc;
           DeserializationError err = deserializeJson(doc, msg);
           if (!err) {
              if (topic == mqtt_topic_sub_cmd || topic == mqtt_topic_sub_broadcast) {
                 ...
                 if (status == "check_in") {
                    ...
                    drawScreen("V " + name, "Check In " + time);
                    digitalWrite(LED_GREEN, HIGH);
                    beep(1, 100);
                 } 
                    ...
                    String time = doc["time"];
                    drawScreen("V " + name, "Out " + time + " (" + duration + ")");
                    digitalWrite(LED_GREEN, HIGH);
                    beep(2, 100);
                 }
...
void loop() {
  ...
  // State Machine untuk layar LCD
  if (currentState == DISPLAY_RESULT && (now - stateTimer > 3000)) {
     digitalWrite(LED_GREEN, LOW);
  ...
  if ((currentState == PROCESSING || currentState == REGISTER_PROCESSING) && (now - stateTimer > 5000)) {
  ...
  // Deteksi Kartu RFID
  if (isMqttConnected && (currentState == IDLE || currentState == REGISTER_MODE)) {
