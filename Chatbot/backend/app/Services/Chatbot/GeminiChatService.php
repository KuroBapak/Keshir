...
class GeminiChatService
{
    ...
    /**
     * Send a message to Gemini with function calling support and role context.
     */
    public function chat(string $userMessage, array $conversationHistory = [], string $role = 'customer'): array
    {
        if (empty($this->apiKey)) {
            return [
                'success' => false,
                'message' => 'Maaf, layanan chatbot sedang tidak tersedia. Silakan hubungi staff kami untuk bantuan. 🙏',
        ...
        try {
            ...
            $response = $this->callGemini($contents, $role);
            ...
            foreach ($parts as $part) {
                if (isset($part['functionCall'])) {
                    ...
                    $finalResponse = $this->callGemini($contents, $role);

                    if ($finalResponse['success']) {
                        $finalText = $finalResponse['data']['candidates'][0]['content']['parts'][0]['text'] ?? '';
                        return [
                            'success' => true,
                            'message' => $finalText,
                            'function_called' => $part['functionCall']['name'],
                        ];
                    }
            ...
            $textResponse = $parts[0]['text'] ?? 'Maaf, saya tidak bisa memproses permintaan Anda saat ini.';
            return ['success' => true, 'message' => $textResponse];

        } catch (\Exception $e) {
            Log::error('GeminiChatService error: ' . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
    ...
    private function buildContents(string $userMessage, array $conversationHistory): array
    {
        $contents = [];
    ...
    private function callGemini(array $contents, string $role): array
    {
        $payload = [
            'contents' => $contents,
            'tools' => [$this->getFunctionDeclarations($role)],
            'systemInstruction' => [
                'parts' => [['text' => $this->getSystemPrompt($role)]],
            ],
        ];
        ...
        if (!$response->successful()) {
            ...
            if ($response->status() === 429) {
                return [
                    'success' => false,
                    'message' => 'Chatbot sedang sibuk melayani pelanggan lain. Silakan coba lagi dalam beberapa detik. ⏳',
                    'error' => 'Rate limited',
                ];
            }
            ...
            return [
                'success' => false,
                'message' => 'Maaf, terjadi gangguan pada layanan AI. Silakan coba lagi nanti. 🙏',
                'error' => $errorMessage,
            ];
        }
...
- Jangan mengarang data menu atau harga — selalu gunakan fungsi yang tersedia.
...
PROMPT;
    ...
    private function getFunctionDeclarations(string $role): array
    {
        $allFunctions = [
                [
        ...
        $declarations = $role === 'customer' 
            ? array_filter($allFunctions, fn($fn) => $fn['name'] !== 'check_stock_status')
            : $allFunctions;

        return [
            'functionDeclarations' => array_values($declarations)
        ];
    }
